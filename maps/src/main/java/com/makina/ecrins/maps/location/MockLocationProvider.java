 ...
 */
public class MockLocationProvider implements IMathConstants, IGeoConstants {
    ...
    public MockLocationProvider(Context pContext) {
        ...
        mLocationManager.addTestProvider(MOCK_LOCATION_PROVIDER, false, false, false, false, true, true, true, Criteria.POWER_LOW, Criteria.ACCURACY_FINE);
        mLocationManager.setTestProviderEnabled(MOCK_LOCATION_PROVIDER, true);
        mCurrentLocation = null;
    }

    public void enableProvider(boolean enabled) {
        if (DebugUtils.isDebuggable(mContext) && DebugUtils.hasAccessMockLocationPermission(mContext) && (mLocationManager.getProvider(MOCK_LOCATION_PROVIDER) != null)) {
            mLocationManager.setTestProviderEnabled(MOCK_LOCATION_PROVIDER, enabled);
        }
    }
    ...
    public void pushLocation(Geolocation geolocation) {
        Log.d(getClass().getName(), "pushLocation " + geolocation.toString());
        ...
        if (isProviderEnabled()) {
            ...
            Log.w(getClass().getName(), "pushLocation : provider '" + MOCK_LOCATION_PROVIDER + "' is not enabled !");
        }
    }
     ...
     * <dl>
     * <dt>Formula used<dt>
     * <dd>φ2 = asin(sin(φ1).cos(d/R) + cos(φ1).sin(d/R).cos(θ))<br/>λ2 = λ1 + atan2(sin(θ).sin(d/R).cos(φ1), cos(d/R) − sin(φ1).sin(φ2))</dd>
     * <dt>where</dt>
     * <dd>φ is latitude, λ is longitude, θ is the bearing (in radians, clockwise from north), d is the distance traveled, R is the earth’s radius (d/R is the angular distance, in radians)</dd>
     * </dl>
     * </p>
     *
    ...
    public void pushRandomLocationFromCurrentLocation(double distance) {
        if (this.mCurrentLocation == null) {
            Log.w(getClass().getName(), "pushNewLocationFromCurrentLocation, failed to push new location : the current location is not defined yet");
        }
        else {
            ...
            final double lon2 = lon1 + Math.atan2(Math.sin(bearing) * Math.sin(dR) * Math.cos(lat1), Math.cos(dR) - Math.sin(lat1) * Math.sin(lat2));
            ...
            if (isProviderEnabled()) {
                ...
                Log.w(getClass().getName(), "pushRandomLocationFromCurrentLocation : provider '" + MOCK_LOCATION_PROVIDER + "' is not enabled !");
            }
        }
    }
