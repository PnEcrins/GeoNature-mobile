 ...
 * @see <a href="https://github.com/beworker/pinned-section-listview">https://github.com/beworker/pinned-section-listview</a>
 */
public class PinnedSectionListView
    ...
    public interface PinnedSectionListAdapter
        ...
        void onSectionViewPinned(@NonNull final View view);
    }
    ...
    public void setShadowVisible(boolean visible) {
        ...
        if (mPinnedSection != null) {
            ...
            invalidate(v.getLeft(),
                       v.getTop(),
                       v.getRight(),
                       v.getBottom() + mShadowHeight);
        }
    }
    ...
    void createPinnedShadow(int position) {
        ...
        View pinnedView = getAdapter().getView(position,
                                               pinnedShadow.view,
        ...
        int ws = MeasureSpec.makeMeasureSpec(getWidth() - getListPaddingLeft() - getListPaddingRight(),
                                             MeasureSpec.EXACTLY);
        int hs = MeasureSpec.makeMeasureSpec(heightSize,
                                             heightMode);
        pinnedView.measure(ws,
                           hs);
        pinnedView.layout(0,
                          0,
                          pinnedView.getMeasuredWidth(),
                          pinnedView.getMeasuredHeight());
        mTranslateY = 0;

        if (getAdapter() instanceof PinnedSectionListAdapter) {
            ((PinnedSectionListAdapter) getAdapter()).onSectionViewPinned(pinnedView);
        }
     ...
     */
    void ensureShadowForPosition(int sectionPosition,
                                 int firstVisibleItem,
                                 int visibleItemCount) {
    ...
    int findFirstVisibleSectionPosition(int firstVisibleItem,
        ...
        for (int childIndex = 0; childIndex < visibleItemCount; childIndex++) {
            ...
            if (isItemViewTypePinned(adapter,
                                     viewType)) {
                return position;
            }
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mPinnedSection != null) {
            ...
            int clipHeight = view.getHeight() + (mShadowDrawable == null ? 0 : Math.min(mShadowHeight,
                                                                                        mSectionsDistanceY));
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        // create touch target
        if (action == MotionEvent.ACTION_DOWN && mTouchTarget == null && mPinnedSection != null && isPinnedViewTouched(mPinnedSection.view,
                                                                                                                       x,
                                                                                                                       y)) {
        ...
        if (mTouchTarget != null) {
            if (isPinnedViewTouched(mTouchTarget,
                                    x,
                                    y)) { // forward event to pinned view
    ...
    private boolean isPinnedViewTouched(View view,
                                        float x,
                                        float y) {
        ...
        return mTouchRect.contains((int) x,
                                   (int) y);
    }
    ...
    public static boolean isItemViewTypePinned(ListAdapter adapter,
                                               int viewType) {
