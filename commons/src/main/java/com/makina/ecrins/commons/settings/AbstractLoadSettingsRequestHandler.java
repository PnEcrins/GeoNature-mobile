...
public abstract class AbstractLoadSettingsRequestHandler
    ...
    public AbstractLoadSettingsRequestHandler(Context pContext) {
        ...
        this.mStatus = new RequestHandlerStatus(RequestHandlerStatus.Status.PENDING,
                                                "");
    ...
    protected void handleMessageFromService(Message message) {
        ...
        if (checkMessage(message) && message.getData()
                                            .containsKey(KEY_COMMAND)) {
            switch ((Command) message.getData()
                ...
                case START:
                    ...
                    if (message.getData()
                               .containsKey(KEY_FILENAME)) {
                        new LoadSettingsFromFileAsyncTask(message.getData()).execute();
                    }
                    break;
                case GET_STATUS:
                    message.getData()
                           .putParcelable(KEY_STATUS,
                                          mStatus);

                    if (mAppSettings != null) {
                        message.getData()
                               .putParcelable(KEY_APP_SETTINGS,
                                              mAppSettings);
                    }
    ...
    private class LoadSettingsFromFileAsyncTask
            extends AsyncTask<Void, Void, AbstractAppSettings> {
        ...
        protected AbstractAppSettings doInBackground(Void... params) {
            ...
            mStatus = new RequestHandlerStatus(RequestHandlerStatus.Status.RUNNING,
                                               "");
            ...
            try {
                ...
                FileUtils.getRootFolder(getContext(),
                                        MountPoint.StorageType.INTERNAL)
                         .mkdirs();

                final File settingsJsonFile = FileUtils.getFile(FileUtils.getRootFolder(getContext(),
                                                                                        MountPoint.StorageType.INTERNAL),
        ...
        protected void onPostExecute(AbstractAppSettings abstractAppSettings) {
            ...
            if (abstractAppSettings == null) {
                mStatus = new RequestHandlerStatus(RequestHandlerStatus.Status.FINISHED_WITH_ERRORS,
                                                   this.mData.getString(KEY_FILENAME));
            }
            else {
                mStatus = new RequestHandlerStatus(RequestHandlerStatus.Status.FINISHED,
                                                   this.mData.getString(KEY_FILENAME));

                this.mData.putParcelable(KEY_APP_SETTINGS,
                                         abstractAppSettings);
            }
